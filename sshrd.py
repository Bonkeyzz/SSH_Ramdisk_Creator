...
def download_required_files(args):
    ...
    with open('BuildManifest.plist', 'rb') as build_manifest:
        ...
        restoreramdisk_path:str = build_manifest_plist['BuildIdentities'][0]['Manifest']['RestoreRamDisk']['Info']['Path']
        kernelcache_path:str = build_manifest_plist['BuildIdentities'][0]['Manifest']['RestoreKernelCache']['Info']['Path']
...
def decrypt_boot_stages(ibss_path, ibec_path, kernelcache_path):
    # run_pcmd(f'../{sys_platform}/gaster decrypt {ibss_path} temp_ramdisk/iBSS.dec')
    # run_pcmd(f'../{sys_platform}/gaster decrypt {ibec_path} temp_ramdisk/iBEC.dec')
    ...
    return ibss_decrypted_path, ibec_decrypted_path, kernelcache_path


def patch_files(ibss_decrypted_path, ibec_decrypted_path, kernelcache_path, devicetree_path, ramdisk_path, trustcache_path):
    ...
    run_pcmd(f'../{sys_platform}/img4 -i {kernelcache_path} -o ../final_ramdisk/{args.ios}/{args.product_type}/{args.model}/kernelcache.img4 -M IM4M -T rkrn -P kcache.bpatch {is_linux}')
    run_pcmd(f'../{sys_platform}/img4 -i {devicetree_path} -o ../final_ramdisk/{args.ios}/{args.product_type}/{args.model}/devicetree.img4 -M IM4M -T rdtr')

    run_pcmd(f'../{sys_platform}/img4 -i {trustcache_path} -o ../final_ramdisk/{args.ios}/{args.product_type}/{args.model}/trustcache.img4 -M IM4M -T rtsc')
    run_pcmd(f'../{sys_platform}/img4 -i {ramdisk_path} -o ramdisk.dmg')
    patch_ramdisk('ramdisk.dmg')
    run_pcmd(f'../{sys_platform}/img4 -i ../logo/logo.im4p -o ../final_ramdisk/{args.ios}/{args.product_type}/{args.model}/logo.img4 -A -T rlgo -M IM4M')
...
def patch_ramdisk(ramdisk_path):
    ...
    run_pcmd(f'../{sys_platform}/img4 -i ramdisk.dmg -o ../final_ramdisk/{args.ios}/{args.product_type}/{args.model}/ramdisk.img4 -M IM4M -A -T rdsk')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cpid', '-c', type=str, help='CPID of device (example 0x8000)')
    parser.add_argument('--model', '-m', type=str, help='Model of device (example n71ap)')
    parser.add_argument('--product_type', '-pt', type=str, help='Product type of device (example iPhone8,1)')
    parser.add_argument('--ios', '-i', type=str, help='iOS version for the ramdisk (example 15.7)')
    args = parser.parse_args()

    # if not os.path.isfile(f'{sys_platform}/gaster'):
    ...
    run_pcmd(f"../{sys_platform}/img4tool -e -s ../shsh/{args.cpid}.shsh -m IM4M")

    ibss_path, ibec_path, kernelcache_path, restoreramdisk_path, trustcache_path, devicetree_path = download_required_files(args)
    ibss_decrypted_path, ibec_decrypted_path, kernelcache_path = decrypt_boot_stages(ibss_path, ibec_path, kernelcache_path)
